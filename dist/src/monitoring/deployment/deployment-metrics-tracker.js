...
exports.DeploymentMetricsTracker = void 0;
const events_1 = require("events");
const agentdb_1 = require("agentdb");
class DeploymentMetricsTracker extends events_1.EventEmitter {
    constructor() {
        super();
     ...
     */
    async trackDeploymentStart(deploymentId, environment, service, version, metadata) {
        const event = {
            id: deploymentId,
            timestamp: Date.now(),
            type: 'start',
     ...
     */
    async trackDeploymentSuccess(deploymentId, duration, metadata) {
        const event = {
            id: deploymentId,
            timestamp: Date.now(),
            type: 'success',
     ...
     */
    async trackDeploymentFailure(deploymentId, error, duration, metadata) {
        const event = {
            id: deploymentId,
            timestamp: Date.now(),
            type: 'failure',
     ...
     */
    async trackDeploymentRollback(deploymentId, reason, metadata) {
        const event = {
            id: deploymentId,
            timestamp: Date.now(),
            type: 'rollback',
    ...
    async getDeploymentTrends(days = 7) {
        ...
        const eventsInRange = this.deploymentEvents.filter(event => event.timestamp >= startTime);
        // Group by day
     ...
     */
    async predictDeploymentSuccess(environment, service, version) {
        // Analyze historical data for similar deployments
        const similarDeployments = this.deploymentEvents.filter(event => event.environment === environment &&
        ...
        const successfulDeployments = recentDeployments.filter(event => event.type === 'success').length;
    ...
    async checkForAnomalies(event) {
        ...
        if (event.type === 'failure') {
            const recentFailures = this.deploymentEvents.filter(e => e.type === 'failure' &&
     ...
     */
    async updateAnomalyPattern(key, pattern) {
    ...
    async calculatePipelineHealth() {
        const recentEvents = this.deploymentEvents.filter(event => event.timestamp > Date.now() - (24 * 60 * 60 * 1000));
     ...
     */
    calculateStageHealth(events, stage) {
        // Simplified calculation based on success rates
        const stageEvents = events.filter(event => event.metadata && event.metadata.stage === stage);
    ...
    identifyBottlenecks(events) {
        ...
        const longDeployments = events.filter(event => event.duration && event.duration > 600000);
     ...
     */
    generateRecommendations(overallHealth, bottlenecks) {
...
exports.DeploymentMetricsTracker = DeploymentMetricsTracker;
//# sourceMappingURL=deployment-metrics-tracker.js.map
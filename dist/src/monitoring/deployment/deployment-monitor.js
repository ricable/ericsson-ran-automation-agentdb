...
const dashboard_manager_1 = require("./dashboard-manager");
class DeploymentMonitor extends events_1.EventEmitter {
    constructor() {
        ...
        this.logger = (0, logger_1.createLogger)('DeploymentMonitor');
        this.metricsCollector = new metrics_collector_1.MetricsCollector('deployment');
     ...
     */
    async startDeploymentMonitoring(deploymentId, config) {
        const metrics = {
            deploymentId,
            status: 'pending',
            startTime: new Date(),
     ...
     */
    async setupHealthChecks(deploymentId, healthCheckConfigs) {
        const healthChecks = healthCheckConfigs.map(config => ({
     ...
     */
    async executeHealthChecks(deploymentId) {
        const healthChecks = this.healthChecks.get(deploymentId);
        if (!healthChecks)
            return;
        const metrics = this.activeDeployments.get(deploymentId);
     ...
     */
    async executeApiHealthCheck(healthCheck) {
        try {
     ...
     */
    async executeDatabaseHealthCheck(healthCheck) {
        try {
     ...
     */
    async executeServiceHealthCheck(healthCheck) {
        try {
     ...
     */
    async executeNetworkHealthCheck(healthCheck) {
        try {
     ...
     */
    async collectPerformanceImpact(deploymentId) {
        const metrics = this.activeDeployments.get(deploymentId);
        if (!metrics)
            return;
        try {
     ...
     */
    async checkDeploymentAlerts(deploymentId) {
        const metrics = this.activeDeployments.get(deploymentId);
        if (!metrics)
            return;
     ...
     */
    async analyzeFailurePatterns(deploymentId) {
        const metrics = this.activeDeployments.get(deploymentId);
        if (!metrics)
            return;
        const healthChecks = this.healthChecks.get(deploymentId) || [];
     ...
     */
    async analyzePartialSuccessPatterns(deploymentId) {
        const metrics = this.activeDeployments.get(deploymentId);
        if (!metrics)
            return;
     ...
     */
    calculateAverageSuccessRate(deployments) {
     ...
     */
    calculateAverageDeploymentDuration(deployments) {
     ...
     */
    calculatePerformanceImpacts(deployments) {
        if (deployments.length === 0)
     ...
     */
    async completeDeploymentMonitoring(deploymentId, status) {
        const metrics = this.activeDeployments.get(deploymentId);
        if (!metrics)
            return;
        metrics.status = status;

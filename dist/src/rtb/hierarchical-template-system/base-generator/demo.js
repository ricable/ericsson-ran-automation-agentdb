...
exports.generateSampleData = exports.runBaseTemplateDemo = void 0;
const index_1 = require("./index");
const performance_monitor_1 = require("./performance-monitor");
async function runBaseTemplateDemo() {
    ...
    try {
        ...
        performance_monitor_1.performanceMonitor.startStage('quick_generation');
        try {
            const result1 = await (0, index_1.generateBaseTemplatesQuick)('./data/MPnh.xml', // XML schema file
            ...
            performance_monitor_1.performanceMonitor.endStage('quick_generation');
            ...
            performance_monitor_1.performanceMonitor.recordError('quick_generation', 'generation_failed', String(error), 'high');
            console.error(`❌ Quick generation failed: ${error}`);
        }
        // Example 2: Advanced configuration with multiple sources
        console.log('\n📖 Example 2: Advanced Template Generation');
        console.log('-'.repeat(40));
        performance_monitor_1.performanceMonitor.startStage('advanced_generation');
        try {
            const orchestrator = new index_1.BaseTemplateOrchestrator({
                xmlParsing: {
                    streaming: true,
                    batchSize: 500,
            ...
            performance_monitor_1.performanceMonitor.endStage('advanced_generation');
            ...
            performance_monitor_1.performanceMonitor.recordError('advanced_generation', 'generation_failed', String(error), 'high');
            console.error(`❌ Advanced generation failed: ${error}`);
        }
        // Example 3: Performance analysis
        console.log('\n📖 Example 3: Performance Analysis');
        console.log('-'.repeat(40));
        performance_monitor_1.performanceMonitor.startStage('performance_analysis');
        try {
            // Simulate processing large dataset
            const mockResult = await simulateLargeDatasetProcessing();
            performance_monitor_1.performanceMonitor.endStage('performance_analysis');
            console.log(`✅ Processed ${mockResult.processedItems} items`);
            console.log(`📊 Throughput: ${mockResult.throughput.toFixed(2)} items/second`);
        }
        catch (error) {
            performance_monitor_1.performanceMonitor.recordError('performance_analysis', 'analysis_failed', String(error), 'medium');
        ...
        const metrics = performance_monitor_1.performanceMonitor.stopMonitoring();
        console.log('\n📊 Performance Report');
        console.log('='.repeat(60));
        console.log(performance_monitor_1.performanceMonitor.generateReport());
        // Save detailed report
        const fs = require('fs').promises;
        try {
            await fs.writeFile('./output/performance-report.md', performance_monitor_1.performanceMonitor.generateReport());
            await fs.writeFile('./output/performance-metrics.json', performance_monitor_1.performanceMonitor.generateJSON());
...
exports.runBaseTemplateDemo = runBaseTemplateDemo;
/**
...
async function simulateLargeDatasetProcessing() {
    ...
    for (let i = 0; i < totalItems; i += batchSize) {
        ...
        if (Math.random() < 0.001) {
            performance_monitor_1.performanceMonitor.recordError('batch_processing', 'simulation_error', `Simulated error at item ${i}`, 'low');
        }
        // Simulate occasional warnings
        if (Math.random() < 0.005) {
            performance_monitor_1.performanceMonitor.recordWarning('batch_processing', `Processing slow at item ${i}`, 'Consider optimizing batch size');
        }
    }
    const duration = Date.now() - startTime;
    const throughput = processedItems / (duration / 1000);
    // Record custom metrics
    performance_monitor_1.performanceMonitor.recordCustomMetric('total_simulation_items', totalItems);
    performance_monitor_1.performanceMonitor.recordCustomMetric('batch_size', batchSize);
    performance_monitor_1.performanceMonitor.recordCustomMetric('simulation_duration_ms', duration);

...
exports.BaseTemplateGenerator = void 0;
class BaseTemplateGenerator {
     ...
     */
    async generateBaseTemplates(xmlResult, csvResult, config = this.getDefaultConfig()) {
     ...
     */
    async generateBaseTemplateForMOClass(moClass, parameters, config) {
     ...
     * Generate feature-specific templates
     */
    async generateFeatureSpecificTemplates(parameters, config) {
     ...
     */
    generateConfigurationValue(parameter, config) {
     ...
     */
    generateIntelligentDefault(parameter) {
     ...
     */
    generateCustomFunctions(parameters, moClass) {
        const functions = [];
     ...
     */
    generateParameterValidation(parameters) {
        const criticalParams = parameters.filter(p => p.constraints &&
     ...
     */
    generateOptimizationLogic(parameters) {
        return [
     ...
     */
    generateConditions(parameters) {
        const conditions = {};
        // Add common conditions based on parameter groups
        const energyParams = parameters.filter(p => p.name.toLowerCase().includes('energy') ||
        ...
        const capacityParams = parameters.filter(p => p.name.toLowerCase().includes('capacity') ||
     ...
     */
    generateEvaluations(parameters) {
        const evaluations = {};
     ...
     */
    async applyCognitiveOptimizations(templates) {
     ...
     */
    async applyStrangeLoopOptimization(template) {
     ...
     * Apply temporal reasoning enhancements
     */
    async applyTemporalReasoning(template) {
     ...
     * Apply self-referential optimization
     */
    async applySelfReferentialOptimization(template) {
        ...
        try {
            ...
            if (template.custom) {
                for (const func of template.custom) {
                    if (!func.name)
                        errors.push('Custom function missing name');
     ...
     */
    async mergeParameters(xmlParameters, csvParameters) {
     ...
     * Group parameters by MO class
     */
    groupParametersByMOClass(parameters) {
     ...
     */
    groupParametersByFeature(parameters) {
        const groups = new Map();
        for (const parameter of parameters) {
     ...
     */
    extractFeatureFromParameter(parameter) {

...
const merge_validator_1 = require("./merge-validator");
...
class TemplateMerger {
    constructor() {
        ...
        this.mergeValidator = new merge_validator_1.MergeValidator();
        this.mergeCache = new Map();
        this.logger = new logger_1.Logger('TemplateMerger');
    }
    /**
     * Merge multiple templates with inheritance resolution
     */
    async mergeTemplates(templates, options = {}) {
        const mergeOptions = {
        ...
        try {
            ...
            // Resolve conflicts
            const resolvedTemplates = await this.resolveConflicts(inheritanceChain.templates, conflicts, context);
            // Perform the actual merge
            const mergedTemplate = await this.performMerge(resolvedTemplates, context);
            // Validate result if requested
            if (mergeOptions.validateResult) {
                const validationResult = await this.mergeValidator.validateMergedTemplate(mergedTemplate, context);
     ...
     */
    async buildInheritanceChain(templates) {
        ...
        const visit = async (templateId, depth = 0) => {
     ...
     */
    getInheritanceDepth(template, graph) {
        const templateId = template.meta?.description || template.meta?.source || 'unknown';
        ...
        return 1 + Math.max(...parents.map(parentId => {
            ...
            return parentTemplate ? this.getInheritanceDepth({ meta: { description: parentId } }, graph) : 0;
        }));
    }
    /**
     * Resolve conflicts using resolution engine
     */
    async resolveConflicts(templates, conflicts, context) {
        ...
        for (const conflict of conflicts) {
            try {
                ...
                if (resolution.resolved) {
                    ...
                    const templateIndex = resolvedTemplates.findIndex(t => t.meta?.description === conflict.templates[conflict.resolvedTemplateIndex]);
                    if (templateIndex !== -1) {
                        resolvedTemplates[templateIndex] = this.applyResolution(resolvedTemplates[templateIndex], conflict, resolution.value);
                    }
     ...
     */
    applyResolution(template, conflict, resolutionValue) {
     ...
     */
    async performMerge(templates, context) {
        ...
        mergedTemplate.custom = this.mergeCustomFunctions(templates.map(t => t.custom || []));
        // Merge conditions
        mergedTemplate.conditions = this.mergeConditions(templates.map(t => t.conditions || {}));
        // Merge evaluations
        mergedTemplate.evaluations = this.mergeEvaluations(templates.map(t => t.evaluations || {}));
        // Merge metadata
        if (context.options.preserveMetadata) {
            mergedTemplate.meta = this.mergeMetadata(templates.map(t => t.meta).filter(Boolean));
        }
     ...
     * Merge two templates
     */
    async mergeTwoTemplates(base, override, context) {
        const merged = {
            ...base,
     ...
     */
    mergeCustomFunctions(functionsArray) {
     ...
     */
    mergeConditions(conditionsArray) {
        const merged = {};
     ...
     */
    mergeEvaluations(evaluationsArray) {
        const merged = {};
     ...
     */
    generateCacheKey(templates, options) {
        const templateHashes = templates.map(t => t.meta?.description || t.meta?.source || 'unknown').sort().join('|');

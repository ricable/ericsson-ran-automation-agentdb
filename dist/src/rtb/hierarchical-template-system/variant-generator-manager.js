...
exports.VariantGeneratorManager = void 0;
const variant_generator_core_1 = require("./variant-generator-core");
const urban_variant_1 = require("./variant-generators/urban-variant");
const mobility_variant_1 = require("./variant-generators/mobility-variant");
const sleep_variant_1 = require("./variant-generators/sleep-variant");
class VariantGeneratorManager {
    constructor() {
        // Performance tracking
        this.generationHistory = [];
        this.coreGenerator = new variant_generator_core_1.VariantGeneratorCore();
        this.urbanGenerator = new urban_variant_1.UrbanVariantGenerator();
        this.mobilityGenerator = new mobility_variant_1.MobilityVariantGenerator();
        this.sleepGenerator = new sleep_variant_1.SleepVariantGenerator();
    }
    /**
     * Generate a single variant based on deployment context
     */
    async generateVariant(baseTemplateName, context) {
        ...
        try {
            switch (context.primaryScenario) {
                case 'urban':
                    ...
                    template = this.urbanGenerator.generateUrbanVariant(baseTemplateName, context.urbanContext, context.globalSettings);
                    variantType = 'urban';
                    break;
                case 'mobility':
                    ...
                    template = this.mobilityGenerator.generateMobilityVariant(baseTemplateName, context.mobilityContext, context.globalSettings);
                ...
                case 'sleep':
                    ...
                    template = this.sleepGenerator.generateSleepVariant(baseTemplateName, context.sleepContext, context.globalSettings);
                    variantType = 'sleep';
                    break;
                case 'hybrid':
     ...
     */
    async generateHybridVariant(baseTemplateName, context) {
        if (!context.hybridConfig) {
        ...
        for (const scenario of context.hybridConfig.scenarios) {
            ...
            switch (scenario) {
                case 'urban':
                    if (!context.urbanContext)
                        continue;
                    variantTemplate = this.urbanGenerator.generateUrbanVariant(baseTemplateName, context.urbanContext, context.globalSettings);
                    break;
                case 'mobility':
                    if (!context.mobilityContext)
                        continue;
                    variantTemplate = this.mobilityGenerator.generateMobilityVariant(baseTemplateName, context.mobilityContext, context.globalSettings);
                    break;
                case 'sleep':
                    if (!context.sleepContext)
                        continue;
                    variantTemplate = this.sleepGenerator.generateSleepVariant(baseTemplateName, context.sleepContext, context.globalSettings);
                    break;
                default:
                    continue;
            }
     ...
     */
    mergeVariants(variants, weights, baseTemplate) {
        const mergedTemplate = { ...baseTemplate };
        ...
        variants.forEach((variant, index) => {
            ...
            this.mergeConfigurationWeighted(mergedConfiguration, variant.configuration, weight);
     ...
     */
    mergeConfigurationWeighted(target, source, weight) {
     ...
     */
    async generateBatchVariants(baseTemplateName, contexts) {
        const results = {};
     ...
     */
    estimatePerformance(template, context) {
...
exports.VariantGeneratorManager = VariantGeneratorManager;
//# sourceMappingURL=variant-generator-manager.js.map
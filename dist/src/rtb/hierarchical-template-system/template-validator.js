...
exports.TemplateValidator = exports.ValidationSeverity = void 0;
/**
...
class TemplateValidator {
    constructor(registry, config = {}) {
        ...
        this.config = {
            ...
            maxConfigurationSize: config.maxConfigurationSize ?? 1024 * 1024,
     ...
     */
    async validateTemplate(templateName, template, priority, inheritanceChain) {
     ...
     */
    async validateInheritanceChain(chain) {
        const errors = [];
        const warnings = [];
     ...
     */
    generateValidationCacheKey(templateName, template, priority) {
     ...
     */
    validateTemplateStructure(context) {
        const results = [];
     ...
     */
    validateCustomFunctions(context) {
        const results = [];
     ...
     */
    validateInheritanceStructure(context) {
        const results = [];
     ...
     */
    validatePriorityLevels(context) {
        const results = [];
     ...
     */
    validatePriorityConsistency(context) {
        const results = [];
     ...
     */
    validateConfigurationSize(context) {
        const results = [];
        const { template } = context;
        if (!template.configuration)
            return results;
     ...
     */
    validateParameterNames(context) {
        const results = [];
        const { template } = context;
        if (!template.configuration)
            return results;
     ...
     */
    validatePerformanceImpact(context) {
        const results = [];
        const { template } = context;

...
exports.DetailedParameterValidator = void 0;
class DetailedParameterValidator {
    ...
    performCrossParameterValidations(parameters) {
    ...
    validateHierarchicalDependencies(parameters, crossResults) {
        ...
        parameters.forEach(param => {
            ...
            const potentialChildren = parameters.filter(p => p.hierarchy && p.hierarchy.length > param.hierarchy.length &&
    ...
    validateReservedByRelationships(parameters, crossResults) {
        if (!this.context.reservedByHierarchy)
            return;
        const paramByName = new Map(parameters.map(p => [p.name, p]));
    ...
    validateCardinalityConstraints(sourceParam, targetParam, constraints, crossResults) {
        ...
        if (constraints.validatorType === 'range') {
            const minRule = constraints.rules.find((r) => r.type === 'min');
            const maxRule = constraints.rules.find((r) => r.type === 'max');
            if (minRule && maxRule) {
    ...
    validateLDNNavigationConflicts(parameters, crossResults) {
        if (!this.context.ldnHierarchy)
            return;
        // Find parameters with conflicting LDN paths
        const conflicts = new Map();
        parameters.forEach(param => {
            if (!param.navigationPaths)
                return;
    ...
    validateNamingConflicts(parameters, crossResults) {
    ...
    validateTypeCompatibility(parameters, crossResults) {
    ...
    checkTypeCompatibility(parentParam, childParam) {
        // Implement type compatibility logic
        const typeCompatibilityRules = {
    ...
    addToCrossResult(crossResults, paramName, result) {
    ...
    generateSummary(detailedResults) {
    ...
    validateConstraint(constraint, type) {
...
exports.DetailedParameterValidator = DetailedParameterValidator;
//# sourceMappingURL=detailed-parameter-validator.js.map
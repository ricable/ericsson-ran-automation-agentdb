...
exports.CmeditCommandParser = void 0;
class CmeditCommandParser {
    constructor(moHierarchy) {
        ...
        this.FDN_COMPONENT_PATTERN = /([^=,\(\)]+)(?:=([^=,\(\)]+))?(?:\(([^)]+)\))?/g;
        this.WILDCARD_PATTERN = /[*?]/;
        this.ATTRIBUTE_PATTERN = /([^.]+(?:\.[^.]+)*)(?:\.\[(\d+)\])?/;
    }
    /**
     * Parse a cmedit command string into structured command object
     */
    parseCommand(commandString, context) {
     ...
     */
    parseBatchCommands(commandStrings, context) {
        const commands = [];
        const errors = [];
     ...
     */
    generateFromTemplate(template, commandType, targetFDN, options) {
     ...
     */
    parseCommandStructure(command, type) {
     ...
     */
    identifyMOClass(componentName) {
        // Common MO class mappings
        const moClassMappings = {
            'MeContext': 'MeContext',
     ...
     */
    determineComponentType(name, value) {
     ...
     */
    analyzeCardinality(componentName, index) {
     ...
     * Validate FDN path
     */
    validateFDNPath(path, components) {
        const errors = [];
        const warnings = [];
     ...
     */
    generateAlternativePaths(components) {
        const alternatives = [];
     ...
     */
    validateCommandSyntax(command, type) {
     ...
     */
    buildCommandContext(partialContext, commandType, fdnPath) {
        return {
     ...
     */
    extractParametersFromTemplate(template) {
     ...
     */
    buildCommandString(type, target, parameters, options) {
        ...
        if (options) {
            ...
            if (options.scopeFilter)
                command += ` --scopefilter (${options.scopeFilter})`;
...
exports.CmeditCommandParser = CmeditCommandParser;

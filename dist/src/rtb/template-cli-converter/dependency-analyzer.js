...
class DependencyAnalyzer {
     ...
     */
    async analyze(commands, context) {
     ...
     */
    createDependencyGraph(commands, context) {
        const nodes = [];
        const edges = [];
        // Create nodes
     ...
     */
    createExecutionLevels(dependencyGraph, criticalPath) {
        const levels = [];
        const processed = new Set();
        const commandToLevel = new Map();
     ...
     */
    generateOptimizations(dependencyGraph, executionLevels, context) {
        const optimizations = [];
     ...
     */
    createExecutionLevel(level, commands, dependencyGraph) {
     ...
     * Helper methods
     */
    generateCacheKey(commands, context) {
    ...
    calculateCycleSeverity(cycleLength) {
        if (cycleLength <= 2)
            return 'critical';
        if (cycleLength <= 4)
            return 'high';
    ...
    generateCycleResolutions(cycle) {
        return [
    ...
    canExecuteInParallel(nodes, dependencyGraph) {
        // Check if there are any dependencies between nodes in this level
        for (let i = 0; i < nodes.length; i++) {
            for (let j = i + 1; j < nodes.length; j++) {
                const hasDependency = dependencyGraph.edges.some(edge => (edge.from === nodes[i].id && edge.to === nodes[j].id) ||
    ...
    findMergeOpportunities(dependencyGraph) {
        const optimizations = [];
        // Look for consecutive SET commands on the same target
        const setCommandsByTarget = new Map();
        for (const node of dependencyGraph.nodes) {
    ...
    findReorderOpportunities(dependencyGraph, executionLevels) {
        const optimizations = [];
    ...
    findRemovalOpportunities(dependencyGraph) {
        const optimizations = [];
     ...
     */
    areRelatedCommands(source, target) {

...
exports.LDNStructureParser = void 0;
const fs_1 = require("fs");
class LDNStructureParser {
    constructor() {
        this.patterns = [];
        this.hierarchy = {
            rootPatterns: [],
    ...
    extractCardinalityFromPath(components) {
    ...
    isValidParentChild(parent, child) {
    ...
    generateDescription(components) {
    ...
    generateNavigationPaths(pattern) {
    ...
    generateFDNPath(lDNPattern, specificValues = {}) {
    ...
    getFullPathForPattern(pattern) {
        return pattern.path.join(',');
    }
    getParentPath(pattern) {
        if (pattern.path.length > 1) {
            return pattern.path.slice(0, -1).join(',');
        }
        return null;
    }
    getChildPaths(pattern) {
        return this.patterns.filter(p => {
            if (p.path.length <= pattern.path.length)
                return false;
...
exports.LDNStructureParser = LDNStructureParser;
//# sourceMappingURL=ldn-structure-parser.js.map
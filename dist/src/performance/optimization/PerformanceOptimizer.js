...
class PerformanceOptimizer extends events_1.EventEmitter {
    constructor(bottleneckDetector, swarmMonitor) {
        super();
    ...
    generateOptimizationRecommendations(metrics, bottlenecks, topology, health) {
    ...
    isStrategyApplicable(strategy, bottleneck, metrics) {
    ...
    createRecommendationFromStrategy(strategy, bottleneck, metrics) {
    ...
    extractRelevantMetrics(metrics, strategy) {
        const relevant = {};
    ...
    calculateTargetMetrics(currentMetrics, strategy) {
        const targetMetrics = {};
    ...
    generateProactiveRecommendations(metrics, topology, health) {
        const recommendations = [];
    ...
    createOptimizationPlans(recommendations, bottlenecks) {
        const plans = [];
    ...
    convertRecommendationToStrategy(recommendation) {
        return {
    ...
    calculateTotalImpact(strategies) {
        const totalPerformanceImprovement = strategies.reduce((sum, s) => sum + s.estimatedImpact.performanceImprovement, 0) / strategies.length;
        const totalResourceSavings = strategies.reduce((sum, s) => sum + s.estimatedImpact.resourceSavings, 0) / strategies.length;
        const totalImplementationTime = strategies.reduce((sum, s) => sum + s.implementation.duration, 0);
    ...
    prioritizeOptimizationPlans(plans) {
        return plans.sort((a, b) => {
            // Prioritize by overall impact
            const impactScore = (plan) => plan.estimatedTotalImpact.performanceImprovement * 0.6 +
    ...
    async executeOptimizationPlan(planId) {
        ...
        try {
            ...
            for (const strategyId of plan.executionOrder) {
                ...
                const dependenciesMet = dependencies.every(depId => results.some(r => r.strategyId === depId && r.status === 'completed'));
                if (!dependenciesMet) {
    ...
    async executeOptimizationStrategy(strategy, plan) {
        const result = {
            planId: plan.id,
    ...
    async validateActionResult(action) {
    ...
    calculateOptimizationImpact(beforeMetrics, afterMetrics) {
        ...
        const performanceChange = this.calculateMetricChange(beforeMetrics.systemHealth.overallSystemScore, afterMetrics.systemHealth.overallSystemScore);
        const resourceChange = this.calculateResourceChange(beforeMetrics.resourceUsage, afterMetrics.resourceUsage);
        const errorRateChange = this.calculateMetricChange(beforeMetrics.systemHealth.errorRate, afterMetrics.systemHealth.errorRate);
        return {
    ...
    calculateMetricChange(before, after) {
        if (before === 0)
            return 0;
        return ((after - before) / before) * 100;
    }
    calculateResourceChange(before, after) {
    ...
    async rollbackOptimization(strategyId) {

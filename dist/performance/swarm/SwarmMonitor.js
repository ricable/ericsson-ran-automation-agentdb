...
class SwarmMonitor extends events_1.EventEmitter {
    constructor() {
        super();
        this.agents = new Map();
        this.tasks = new Map();
        ...
        this.maxCommunicationHistory = 10000;
    ...
    getSwarmMetrics() {
        ...
        const taskPerformance = {
            ...
            throughput: completedTasks.length / 5,
    ...
    calculateTaskDistributionBalance(agents) {
        if (agents.length === 0)
            return 0;
        const taskCounts = agents.map(agent => agent.status === 'busy' ? 1 : 0);
    ...
    estimateConsensusSpeed(communications) {
        const coordinationMessages = communications.filter(comm => comm.messageType === 'coordination');
    ...
    calculateSynchronizationAccuracy(communications) {
        const syncMessages = communications.filter(comm => comm.messageType === 'data_sync' || comm.messageType === 'coordination');
    ...
    calculateNetworkUtilization(communications) {
    ...
    estimateDiskIOPS(tasks) {
        // This is a simplified estimation based on task complexity
        const ioIntensiveTasks = tasks.filter(task => task.type.includes('database') ||
    ...
    getSwarmHealth() {
        ...
        const activeAgents = Array.from(this.agents.values())
            .filter(agent => agent.lastHeartbeat > fiveMinutesAgo);
        const healthyAgents = activeAgents.filter(agent => agent.status !== 'failed' && agent.status !== 'recovering').length;
    ...
    countActiveConnections(communications) {
        ...
        recentCommunications.forEach(comm => {
            ...
            if (!connections.has(connectionKey)) {
                connections.set(connectionKey, {
                    ...
                    bandwidth: comm.dataSize * 8 / comm.latency,
    ...
    calculateTopologyEfficiency(agents, connections) {
        if (agents.length === 0)
            return 1;
    ...
    identifyBottleneckNodes(agents, connections) {
        const bottlenecks = [];
        agents.forEach(agent => {
            ...
            const degree = connections.filter(conn => conn.source === agent.id || conn.target === agent.id).length;
    ...
    generateOptimizationSuggestions(agents, connections) {
        const suggestions = [];
    ...
    getTasks(filter) {
        ...
        if (filter) {
            ...
            if (filter.timeRange) {
                tasks = tasks.filter(task => task.createdAt >= filter.timeRange.start &&
    ...
    getCommunications(filter) {
        ...
        if (filter) {
            ...
            if (filter.timeRange) {
                communications = communications.filter(comm => comm.timestamp >= filter.timeRange.start &&
    ...
    exportMetrics() {
        return {
            ...
            communications: this.communications.slice(-1000),
...
//# sourceMappingURL=SwarmMonitor.js.map